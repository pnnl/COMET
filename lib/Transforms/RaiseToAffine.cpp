// from https://github.com/llvm/Polygeist/blob/main/lib/polygeist/Passes/RaiseToAffine.cpp
...
namespace {
    struct RaiseSCFToAffine : public mlir::impl::SCFRaiseToAffineBase<RaiseSCFToAffine> {
...
struct ForOpRaising : public OpRewritePattern<scf::ForOp> {
    ...
    int64_t getStep(mlir::Value value) const {
        auto cstOp = value.getDefiningOp<ConstantIndexOp>();
        if (cstOp)
            return cstOp.value();
        else
            return 1;
    ...
    LogicalResult matchAndRewrite(scf::ForOp loop,
        ...
        if (isAffine(loop)) {
            ...
            auto affineLoop = rewriter.create<AffineForOp>(
...
struct ParallelOpRaising : public OpRewritePattern<scf::ParallelOp> {
    ...
    LogicalResult matchAndRewrite(scf::ParallelOp loop,
        ...
        auto affineLoop = rewriter.create<AffineParallelOp>(
        ...
        // The terminator is added if the iterator args are not provided.
        // see the ::build method.
        if (affineLoop.getResults().empty()) {
            auto *affineYieldOp = newBlock.getTerminator();
            rewriter.eraseOp(affineYieldOp);
        }
