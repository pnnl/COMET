...
struct GPUToSPIRVPass final : impl::ConvertGPUToSPIRVBase<GPUToSPIRVPass> {
  explicit GPUToSPIRVPass(bool mapMemorySpace, bool use64bitIndex)
      : mapMemorySpace(mapMemorySpace), use64bitIndex(use64bitIndex) {}
  void runOnOperation() override;

private:
  bool mapMemorySpace, use64bitIndex;
};
...
mlir::createConvertGPUToSPIRVPass2(bool mapMemorySpace, bool use64bitIndex) {
  return std::make_unique<GPUToSPIRVPass>(mapMemorySpace, use64bitIndex);
}
