...
// *********** For debug purpose *********//
// #ifndef DEBUG_MODE_GPUUtils
// #define DEBUG_MODE_GPUUtils
// #endif

#ifdef DEBUG_MODE_GPUUtils
...
void replaceMemrefUsesAndPropagateType(RewriterBase &rewriter, Location loc,
  ...
  while (!worklist.empty()) {
    ...
    //rewriter.replaceUsesWithIf(original, replacement, [&](OpOperand &use) {
      rewriter.replaceUseIf(original, replacement, [&](OpOperand &use) {  // TODO: verify, does this replacement work? 
...
llvm::SmallVector<mlir::linalg::ProcInfo, 2> getSubgroupIdsAndCounts(
  ...
  for (unsigned i = 0; i < numDims; ++i) {
    ...
    if (i == 0) {
      ...
      subgroupId = makeComposedAffineApply(
...
std::array<int64_t, 3> getWorkgroupSize(mlir::func::FuncOp funcOp) {
  ...
  // TODO: get these through config
  workgroupSize[0] = 32; workgroupSize[1] = 1; workgroupSize[2] = 1; 
...
LogicalResult copyToWorkgroupMemory(OpBuilder &b, Value src, Value dst) {
  ...
  mlir::comet::setMarker(copyOp, mlir::comet::getCopyToWorkgroupMemoryMarker());
  return success();
}
...
static void insertInputValueIntoGeneric(Value source, linalg::GenericOp op) {
  ...
  mlir::comet::setMarker(newOp, mlir::comet::getCopyToWorkgroupMemoryMarker());
}
...
void propagateSharedMemoryCopy(func::FuncOp funcOp) {
  ...
  funcOp.walk([&toDelete](memref::CopyOp copyOp) {
    if (mlir::comet::hasMarker(copyOp, mlir::comet::getCopyToWorkgroupMemoryMarker())) {
...
void insertBarriersAroundSharedMemoryCopy(func::FuncOp funcOp) {
  ...
  funcOp.walk([&builder](Operation *copyOp) {
    if (mlir::comet::hasMarker(copyOp, mlir::comet::getCopyToWorkgroupMemoryMarker())) {
      Operation *prevOp = copyOp->getPrevNode();
      if (!prevOp || !mlir::comet::hasMarker(prevOp, mlir::comet::getCopyToWorkgroupMemoryMarker())) {
        builder.setInsertionPoint(copyOp);
        builder.create<gpu::BarrierOp>(copyOp->getLoc());
      }
      Operation *nextOp = copyOp->getNextNode();
      if (!nextOp || !mlir::comet::hasMarker(nextOp, mlir::comet::getCopyToWorkgroupMemoryMarker())) {
