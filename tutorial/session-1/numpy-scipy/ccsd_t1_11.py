...
############# NumPy ##########
def run_numpy_no_opt(v,t2,t1):
	i0 = np.einsum('cdmn,inad,mc->ia', v,t2,t1)
	return i0

def run_numpy_opt(v,t2,t1):
	i0 = np.einsum('cdmn,inad,mc->ia', v,t2,t1, optimize=True)
	return i0

############# COMET ##########
@comet.compile(flags=None)
def run_comet_no_opt(v,t2,t1):
	i0 = comet.einsum('cdmn,inad,mc->ia', v,t2,t1)
	return i0

# @comet.compile(flags="--convert-tc-to-ttgt") #opt1
# @comet.compile(flags="--opt-matmul-tiling --convert-tc-to-ttgt") #opt2
@comet.compile(flags="--opt-bestperm-ttgt --opt-matmul-tiling --opt-matmul-mkernel --opt-dense-transpose --convert-tc-to-ttgt --opt-multiop-factorize") #opt3
def run_comet_opt(v,t2,t1):
	i0 = comet.einsum('cdmn,inad,mc->ia', v,t2,t1)
	return i0

#Inputs
v = np.full([72, 72, 72, 72], 2.3,  dtype=float)
t2 = np.full([72, 72, 72, 72], 3.4,  dtype=float)
t1 = np.full([72, 72], 3.4,  dtype=float)
i0 = np.full([72, 72], 0.0,  dtype=float)
...
for _ in range(num_runs):
	start_time = perf_counter()            # Start the timer	
	expected_result = run_numpy_opt(v,t2,t1)  # Call the function
	end_time = perf_counter()		       # End the timer
...
for _ in range(num_runs):
	start_time = perf_counter()        			# Start the timer
	result_with_jit = run_comet_opt(v,t2,t1)
	end_time = perf_counter()		            # End the timer
