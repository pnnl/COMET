...
@comet.compile(flags="--opt-bestperm-ttgt --opt-matmul-tiling  --opt-matmul-mkernel  --opt-dense-transpose --convert-tc-to-ttgt --convert-to-loops") #opt3
def run_comet_opt(v,t2):
	i0 = comet.einsum('dca,bd->abc', v,t2)
	return i0

#Inputs
v = np.full([mtx_size_large, mtx_size_small, mtx_size_large], 2.3,  dtype=float)
t2 = np.full([mtx_size_large, mtx_size_large], 3.4,  dtype=float)
i0 = np.full([mtx_size_large, mtx_size_large, mtx_size_small], 0.0,  dtype=float)
