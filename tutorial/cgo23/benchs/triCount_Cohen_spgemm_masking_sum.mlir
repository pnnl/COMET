module  {
  func @main() {
    ...
    call @read_input_sizes_2D_lowerTriangle_f64(%c0_i32, %c0, %c1, %1) {filename = "SPARSE_FILE_NAME0"} : (i32, index, index, memref<*xindex>) -> ()
    ...
    call @read_input_2D_lowerTriangle_f64(%c0_i32, %c0, %c1, %9, %11, %13, %15, %17) {filename = "SPARSE_FILE_NAME0"} : (i32, index, index, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xf64>) -> ()
    %18 = memref.alloc() : memref<7xindex>
    %19 = memref.cast %18 : memref<7xindex> to memref<*xindex>
    call @read_input_sizes_2D_upperTriangle_f64(%c1_i32, %c0, %c1, %19) {filename = "SPARSE_FILE_NAME1"} : (i32, index, index, memref<*xindex>) -> ()
    ...
    call @read_input_2D_upperTriangle_f64(%c1_i32, %c0, %c1, %27, %29, %31, %33, %35) {filename = "SPARSE_FILE_NAME1"} : (i32, index, index, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xf64>) -> ()
    %36 = addi %7, %c1 : index
    %37 = muli %7, %25 : index
    ...
    call @read_input_sizes_2D_f64(%c2_i32, %c0, %c1, %49) {filename = "SPARSE_FILE_NAME2"} : (i32, index, index, memref<*xindex>) -> ()
    ...
    call @read_input_2D_f64(%c2_i32, %c0, %c1, %56, %58, %60, %62, %64) {filename = "SPARSE_FILE_NAME2"} : (i32, index, index, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xf64>) -> ()

    /// Changed D's type from f64 to i1, in order to use cmpi
    ...
    scf.for %arg0 = %c0 to %71 step %c1 {
      ...
      scf.for %arg11 = %173 to %174 step %c1 {
        ...
        // %178 = cmpi ne, %177, %c0 : index // if (%177 != 0)
        // scf.if %178 {
      ...
      call @quick_sort(%82, %81) : (memref<*xindex>, index) -> ()
    ...
    %887 = memref.alloc() : memref<1xf64>         // sum global
    memref.store %cst, %887[%c0] : memref<1xf64> // init sum
    ...
    scf.for %arg0 = %c0 to %996 step %c1 {       
      ...
      %900 = memref.load %887[%c0] : memref<1xf64>     // load sum global
      %911 = addf %889, %900 : f64                     // cur NNZ + sum global
      memref.store %911, %887[%c0] : memref<1xf64>     // update global sum
    }
    %72 = call @getTime() : () -> f64
    call @printElapsedTime(%66, %72) : (f64, f64) -> ()


    // %73 = memref.cast %38 : memref<?xindex> to memref<*xindex>
    // call @comet_print_memref_i64(%73) : (memref<*xindex>) -> ()
    // %74 = memref.cast %39 : memref<?xindex> to memref<*xindex>
    // call @comet_print_memref_i64(%74) : (memref<*xindex>) -> ()
    // %75 = memref.cast %40 : memref<?xindex> to memref<*xindex>
    // call @comet_print_memref_i64(%75) : (memref<*xindex>) -> ()
    // %76 = memref.cast %41 : memref<?xindex> to memref<*xindex>
    // call @comet_print_memref_i64(%76) : (memref<*xindex>) -> ()
    // %77 = memref.cast %42 : memref<?xf64> to memref<*xf64>
    // call @comet_print_memref_f64(%77) : (memref<*xf64>) -> ()
    return
  }
  func private @read_input_2D_lowerTriangle_f64(i32, index, index, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xf64>)
  func private @read_input_sizes_2D_lowerTriangle_f64(i32, index, index, memref<*xindex>)
  func private @read_input_2D_f64(i32, index, index, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xf64>)
  func private @read_input_sizes_2D_f64(i32, index, index, memref<*xindex>)
  func private @read_input_2D_upperTriangle_f64(i32, index, index, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xindex>, memref<*xf64>)
  func private @read_input_sizes_2D_upperTriangle_f64(i32, index, index, memref<*xindex>)
  func private @quick_sort(memref<*xindex>, index)
  func private @getTime() -> f64
  func private @printElapsedTime(f64, f64)
