...
@comet.compile(flags=None) # Executes on CPU
def run_cometpy_cpu(A,B):
	C = A @ B

	return C

@comet.compile(flags=None, target='gpu') # Executes on GPU Default block-size(y=8, x=32, r=32)
...
res_cometpy = run_cometpy_default(A,B)
